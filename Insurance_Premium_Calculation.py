...
# Normalize the monetary expression so the hard-constraint coefficients are
# on a similar numerical scale to the goal equations.
model.add_constraint(
    Constraint(
        name="Subsidy_Budget",
        expression=(
...
# The target-surplus value is used only as a numerical scaling factor here;
# dividing both sides by a positive constant leaves the constraint unchanged.
model.add_constraint(
    Constraint(
...
# Express the floor directly as a proportion to keep coefficients near 1.
model.add_constraint(
    Constraint(
